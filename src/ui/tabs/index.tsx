...
export default function Tabs ({ tabs } : Props) {
    ...
    return <div className={Styles.root}>
        <div className={Styles.tabs}>
            {tabs.map((tab, i) => <div className={[Styles.tab, index === i ? Styles.active : ''].join(' ')} key={`tab${tab[0]}`} onClick={() => { setHash(tab[0].toLowerCase()); window.location.hash = tab[0].toLowerCase() }}>
                {tab[0]}
            </div>)}
        </div>
