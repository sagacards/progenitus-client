...
export const createLikesSlice: StoreSlice<LikesStore, CompleteStore> = (
    set,
    get
) => ({
    likes: [],
    ...
    doesLike(token) {
        return (
            get().likes.findIndex(
                x =>
                    x.canister.toText() === token.canister &&
                    x.index === token.index
            ) > -1
        );
    },
    ...
    like(token) {
        ...
        likes.like(token.canister, BigInt(token.index)).then(get().fetchLikes);
    ...
    unlike(token) {
        ...
        likes
            .unlike(token.canister, BigInt(token.index))
            .then(get().fetchLikes);

        set(state => ({
            likes: state.likes.filter(
                x =>
                    !(token.canister === x.canister) &&
                    !(token.index === x.index)
            ),
        }));
    },
