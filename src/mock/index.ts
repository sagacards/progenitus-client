...
export default function makeEvents(count = 3): {
    ...
    const events: { [key: MintingEvent['id']]: MintingEvent } = {};
...
export function makeCollections(count = 3): { [key: string]: Collection } {
...
export function eventFactory(): MintingEvent {
    return {
        ...
        access: Math.random() > 0.5 ? 'private' : 'public',
        price: {
            e8s: 4_00_000_000 + Math.floor(Math.random() * 10) * 1_00_000_000,
        },
        startDate: randomDate(24, Math.random() > 0.5),
...
export function collectionFactory(index?: number): Collection {
    ...
    return {
        ...
        canister:
            SagaCanisters[
                index || Math.floor(SagaCanisters.length * Math.random())
            ],
        description,
    };
}
...
for (const canister of SagaCanisters) {
    ...
    for (let i = 50; i >= 0; i--) {
        l.push({
            ...
            event: {
                type: 'mint',
                timestamp: randomDate(4).toJSDate(),
            } as CAPEvent,
        });
        index++;
    }
...
function randomDate(hourVariance = 4, future = false) {
    return DateTime.fromMillis(
        new Date().getTime() +
            Math.random() * 1000 * 60 * hourVariance * (future ? 1 : -1)
    );
}
